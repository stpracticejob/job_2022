...
1. Для того чтобы постоянно не вводить пароль для закрытого ключа настроим сервис `ssh-agent` который безопастно сохраняет в памяти пароль для закрытого ключа и автоматически его применяет при соедиении с серверами ssh. Для запуска сервиса ssh-agent выполняем следующую команду:
    ```
    net start ssh-agent
    ```
...
1. Добавляем закрытый ключ к сервису `ssh-agent`:
    ...
    ```
    Команда запросит пароль к закрытому ключу, который надо ввести.
...
1. Чтобы не вводить полностью часто употребляемые команды Git можно настроить альясы.
    ```
    ...
    git st
    ```
    Можно создавать более сложные альясы. Вот примеры команд для добавления их в настройки:
    ```
...
1. Для просмотра изменений вашей ветки и ветки `master` можно добавить альяс `diff-master`:
    ```
